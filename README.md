...
- [Структура бд и запросы](#стуктура-бд-и-запросы)
...
- Просмотр задач:
    - Команда в постоянном меню /my_tasks отображает список задач пользователя с пагинацией. В каждой странице показывается по 10 задач. Есть возможность перемещаться вперед и назад по страницам
...
- Редактирование задачи:
    ...
    - Статусы берем из списка (один из атрибутов класса обработчика). Текущий статус задачи выбрать нельзя, только новый (или отменить действие по смене статуса)
...
Запросы в бд осуществляются через ORM SQLAlchemy. В круд классе реализуются операции: создания, чтения, обновления и удаления для пользователей и задач.  
...
- Зарегистрировать приложение и получить [креды]('https://my.telegram.org/auth') для работы с Pyrogram
